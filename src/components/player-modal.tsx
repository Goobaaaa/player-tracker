...
import { getPlayerAssets, calculatePlayerAssetsValue, updatePlayer, addPlayer, deletePlayer, getPlayerMugshots, setProfilePicture, getPlayerProfilePicture, addMugshot, getPlayerMedia, addMedia, getPlayerHouseMedia, addHouseMedia, mockAssets, mockMugshots, mockMedia, mockHouseMedia, getPlayerDocuments, addPlayerDocument, deletePlayerDocument, getPlayerWeapons, mockWeapons } from "@/lib/mock-data";
...
export default function PlayerModal({ player, isOpen, onClose, onPlayerSaved, onPlayerDeleted, isEditMode = false }: PlayerModalProps) {
  ...
  const handleAddMedia = () => {
    if (player && mediaUrl.trim()) {
      const newMedia = addMedia({
        ...
        url: mediaUrl.trim(),
        displayName: mediaDisplayName.trim() || undefined
      });
      if (newMedia) {
        setMedia(getPlayerMedia(player.id));
        setMediaUrl('');
  ...
  const handleAddHouseMedia = () => {
    if (player && houseMediaUrl.trim()) {
      const newHouseMedia = addHouseMedia({
        playerId: player.id,
        filename: `house-media-${Date.now()}`,
        url: houseMediaUrl.trim(),
        displayName: houseMediaDisplayName.trim() || undefined
      });
      if (newHouseMedia) {
        setHouseMedia(getPlayerHouseMedia(player.id));
        setHouseMediaUrl('');
  ...
  const handleAddDocument = () => {
    if (player && documentFile) {
      ...
      const newDocument = addPlayerDocument({
        ...
        url: URL.createObjectURL(documentFile), // In real app, this would be server URL
        isGoogleDoc: false,
                                          ...
                                          <button
                                            onClick={() => {
                                              if (confirm('Remove this image?')) {
                                                // removeVehicleImage(asset.id, imageUrl); // Function removed - Vehicle interface doesn't support additionalImages
                                                // Update local state
                  ...
                  <Button
                    onClick={() => {
                      if (vehicleImageUrl.trim()) {
                        // addVehicleImage(selectedVehicleId, vehicleImageUrl.trim()); // Function removed - Vehicle interface doesn't support additionalImages
